...
$gridcontextmenus['writeElFiles']['code']="
        m.push({
            ...
            text: 'export elements to GPM Package',
...
$gridcontextmenus['copyCoreAssets']['code']="
        m.push({
            className : 'copyCoreAssets', 
            text: 'Copy Core and Assets to GPM Package',
            handler: 'this.copyCoreAssets'
        });
        m.push('-');
";

$gridcontextmenus['copyCoreAssets']['handler'] = 'this.copyCoreAssets';

$gridfunctions['this.copyCoreAssets'] = "
copyCoreAssets: function() {
        MODx.Ajax.request({
            url: this.config.url
            ,params: {
                action: 'mgr/migxdb/process'
				,processaction: 'copycoreassets'
                ,object_id: this.menu.record.id
				,configs: this.config.configs
                ,resource_id: this.config.resource_id
                ,co_id: '[[+config.connected_object_id]]'
...
$gridcontextmenus['resolveTables']['code']="
        m.push({
            className : 'resolveTables', 
            text: 'Add Tables to GPM Config',
            handler: 'this.resolveTables'
        });
        m.push('-');
";

$gridcontextmenus['resolveTables']['handler'] = 'this.resolveTables';

$gridfunctions['this.resolveTables'] = "
resolveTables: function() {
        MODx.Ajax.request({
            url: this.config.url
            ,params: {
                action: 'mgr/migxdb/process'
				,processaction: 'resolvetables'
                ,object_id: this.menu.record.id
				,configs: this.config.configs
                ,resource_id: this.config.resource_id
                ,co_id: '[[+config.connected_object_id]]'
